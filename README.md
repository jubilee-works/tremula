...
tremula bundle · run 20260809T041500Z-3b1f8c · project: sample_project
  ...
  logs: 2 file(s), machine paths removed
...
triaged, one `git apply`-ready patch per mutant, the suite's output per mutant, and
`bundle.json` indexing all of it with a SHA-256 for each. Every patch was offered to
`git apply --check` against the bytes the run measured before it was written, so one
that is missing is one git refused, with the reason in the index. `START_HERE.md`
says which order to read the documents in and how to reproduce one bug; it is the
