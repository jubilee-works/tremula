...
tremulaの正規の配布形態は2つのwheelです。`tremula`がCLIを収め、Python
言語パックはその依存関係として一緒に入ります。wheelを提供するインデッ
クスがある環境では、インストールは1コマンドで済みます:
...
しています。wheelを提供するインデックスがない環境では、このリポジトリを
クローンして計測したいプロジェクトにインストールしてください:
