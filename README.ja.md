...
> **初期リリースです。** 1.0までインターフェースが変更される可能性があります。
...
tremulaはPyPIに2つのwheelとして配布されています。`tremula`がCLIを収め、
Python言語パック`tremula-python`はその依存関係として一緒に入ります:

```sh
uv add --dev tremula
# pipなら: pip install tremula
```

tremulaはPython言語パックとCosmic Rayを通じてPython 3.10以降をサポート
しています。wheelはLinux x86_64とmacOS arm64向けにビルドされています。
それ以外のプラットフォームでは、このリポジトリをクローンして計測したい
プロジェクトにインストールしてください。Rustコアをビルドするため、Rust
ツールチェーンが必要です:
...
どちらの場合も、CLIと言語パックがテストスイートと同じプロジェクト環境に
入ります。

実行はプロジェクト自身のPythonで言語パックを駆動します。`--python <パス>`
でインタプリタを指定でき、指定しなければ有効な仮想環境、次にプロジェ
クトの `.venv` を使います。パックはtremulaが指すインタプリタにインス
トールされている必要があり — `pip install --user` で入れたパッケージは
見えません — スイートはpytestで実行されるため、pytestも同じ環境に必要
です。
...
>
> **社内コードをプロバイダーに送るかどうかは利用者側の判断です。** 組織が
> 所有するコードに対してtremulaを実行する前に、組織のポリシーと
> プロバイダーのデータ利用規約を確認してください — 保持期間、API入力の
> 学習利用の有無、処理リージョンはプロバイダーとプランによって異なります。
> tremulaが送信するのは上記のコマンドが送る内容だけで、送信先はAPIキーが
> 示すプロバイダーだけです。
>
...
サブプロセスプロトコルと四つのファイル契約が拡張点であり、新しいパックを
加えるためにコアの変更は必要ない。
